...
export const AnimationOnScroll = ({
  ...
  const inViewport = useCallback(
    (elementTop: any, elementBottom: any) => {
      return (
  ...
  const visibilityHasChanged = useCallback(
    (
      previousVis: { inViewport: any; onScreen: any },
      currentVis: { inViewport: any; onScreen: any }
    ) => {
      return (
  ...
  const animateInTrigger = useCallback(
    (callback: (arg0: { inViewport: boolean; onScreen: boolean }) => void) => {
      animate(animateIn, () => {
        if (!animateOnce) {
          setStyle({
            animationDuration: `${duration}s`,
